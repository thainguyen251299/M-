<?php
/*
for more information: see languages.txt in the lang folder.
*/
$ShowAllEvents = "Hiển thị tất cả các mục đã đánh dấu";
$Month = "Tháng";
$Hour = "Giờ";
$Minutes = "Phút";
$AddSuccess = "Thêm mới thành công";
$AgendaDeleteSuccess = "Đã xóa";
$NoAgendaItems = "Không có mục nào được đánh dấu";
$ModifInfo = "Các xác lập cho khóa học";
$ModifDone = "Thông tin đã được hiệu chỉnh";
$DelCourse = "Xóa toàn bộ website của khóa học";
$Professors = "Giáo viên";
$Faculty = "Bộ môn";
$Confidentiality = "Bảo mật";
$PrivOpen = "Truy cập có điều kiện hạn chế, đăng kí tự do";
$Forbidden = "Không có quyền";
$CourseAccessConfigTip = "Mặc định, chỉ có người dùng mới có thể truy cập đến khóa học mà mình đã đăng kí. Nếu bạn muốn bảo mật,các đơn giản nhất là cho phép đăng kí trong vòng 1 tuần, yêu cầu chính học sinh tự đăng kí, sau đó không cho phép đăng kí thêm và kiểm tra kẻ xâm nhập có thể có trong danh sách";
$TipLang = "Ngôn ngữ này sẽ được hiển thị đối với tất cả các website khóa học của bạn";
$Vid = "Video";
$Work = "Bài viết của học sinh";
$ProgramMenu = "Chương trình khóa học";
$Stats = "Thống kê";
$UplPage = "Upload trang và tạo liên kết tại trang chủ";
$LinkSite = "Thêm liên kết vào trang chính";
$HasDel = "Đã bị xóa";
$ByDel = "Xóa website khóa học này sẽ xóa vĩnh viễn tất cả dữ liệu(không xóa chúng từ khóa học khác).<p>Bạn có thực sự muốn xóa khóa học này";
$Y = "YES";
$N = "NO";
$DepartmentUrl = "Đường dẫn trang chủ của khoa";
$DepartmentUrlName = "Trường/Khoa";
$BackupCourse = "Lưu trữ khóa học này";
$ModifGroups = "Sửa Nhóm";
$Professor = "Giáo viên";
$DescriptionCours = "Mô tả khóa học";
$ArchiveCourse = "BackUp khóa học";
$RestoreCourse = "Restore khóa học";
$Restore = "Restore";
$CreatedIn = "Tạo mới";
$CreateMissingDirectories = "Creation of missing directories";
$CopyDirectoryCourse = "Copy thư mục khóa học";
$Disk_free_space = "Dung lượng còn trống";
$BuildTheCompressedFile = "Creation of backup file";
$FileCopied = "Đã sao chép file";
$ArchiveLocation = "Vị trí lưu trữ tài liệu";
$SizeOf = "Dung lượng của";
$ArchiveName = "Tên tài liệu";
$BackupSuccesfull = "BackUp thành công";
$BUCourseDataOfMainBase = "BackUp dữ liệu khóa học tại cơ sở dữ liệu chính";
$BUUsersInMainBase = "BackUp dữ liệu người dùng tại cơ sở dữ liệu chính";
$BUAnnounceInMainBase = "BackUp thông báo tại cơ sở dữ liệu chính";
$BackupOfDataBase = "BackUp cơ sở dữ liệu";
$ExpirationDate = "Ngày hết hạn";
$LastEdit = "Chỉnh sửa trước";
$LastVisit = "Lần ghé thăm trước";
$Subscription = "Đăng kí";
$CourseAccess = "Truy cập khóa học";
$ConfirmBackup = "Bạn có thực sự muốn BackUp khóa học này";
$CreateSite = "Tạo website cho khóa học mới";
$RestoreDescription = "Khóa học được lưu trong tệp tin mà bạn có thể lựa chọn dưới đấy<br><br>Khi bạn nhấn vào nút \"Restore\", các kho lưu trữ sẽ được giải nén và tái tạo quá trình";
$RestoreNotice = "Kịch bản này không cho phép có thể tự động khôi phục lại người dùng, nhưng dữ liệu được lưu trữ trong \"users.csv\" cho phép quản trị viên thao tác công việc đó bằng tay";
$AvailableArchives = "Danh sách lưu trữ sẵn sàng";
$NoArchive = "Chưa lưa chọn dữ liệu";
$ArchiveNotFound = "Không tìm thấy dữ liệu";
$ArchiveUncompressed = "Lưu trữ đã được giải nén và cài đặt";
$CsvPutIntoDocTool = "File \"users.csv\" đã được đưa vào công cụ tài liệu.";
$Explanation = "Sau khi bấm OK, một website sẽ được tạo ra với đầy đủ các chuyên mục. Bạn chỉ cần đăng nhập và hiệu chỉnh theo ý muôn.";
$CodeTaken = "Mã này đã tồn tại";
$ExerciceEx = "Bài tập mẫu";
$Antique = "Lịch sử triết học cổ đại";
$SocraticIrony = "Socratic irony is...";
$ManyAnswers = "(nhiều hơn 1 câu trả lời có thể đúng)";
$Ridiculise = "Ridiculise one's interlocutor in order to have him concede he is wrong.";
$NoPsychology = "Số Socrates trớ trêu là không phải là một vấn đề của tâm lý học, nó liên quan đến lập luận.";
$AdmitError = "Admit one's own errors to invite one's interlocutor to do the same.";
$NoSeduction = "Số Socrates trớ trêu là không phải là một chiến lược quyến rũ hoặc một phương pháp dựa trên ví dụ.";
$Force = "Compell one's interlocutor, by a series of questions and sub-questions, to admit he doesn't know what he claims to know.";
$Indeed = "Indeed. Socratic irony is an interrogative method. The Greek \"eirotao\" means \"ask questions\"";
$Contradiction = "Use the Principle of Non Contradiction to force one's interlocutor into a dead end.";
$NotFalse = "This answer is not false. It is true that the revelation of the interlocutor's ignorance means showing the contradictory conclusions where lead his premisses.";
$AddPageHome = "Upload trang và liên kết vào trang chủ";
$ModifyInfo = "Các xác lập của khóa học";
$CourseDesc = "Mô tả khóa học";
$AgendaTitle = "Thứ 3 ngày 11 tháng 12 - Bài học thứ nhất : Newton 18";
$AgendaText = "Giới thiệu chung về Triết lí và phương pháp luận";
$Micro = "Phỏng vấn";
$Google = "Công cụ tìm kiếm nhanh và mạnh mẽ";
$IntroductionTwo = "Trang này dành cho học sinh hoặc nhóm học sinh gửi bài vào website .Nếu không có hình ảnh đính kèm vào nội dung, nên gửi dưới dạng tập tin .htm.";
$AnnouncementEx = "Thông báo mẫu . Chỉ có giáo viên hoặc quản trị viên mới có quyền thông báo tại đây";
$JustCreated = "Bạn vừa tạo xong 1 site khóa học";
$CreateCourseGroups = "Khỏi tạo khóa học nhóm";
$CatagoryMain = "Nhóm diễn đàn chính";
$CatagoryGroup = "Diễn đàn nhóm";
$Ln = "Ngôn ngữ";
$FieldsRequ = "Xin điền đầy đủ các mục";
$Ex = "ví dụ . <i>Lịch sử - văn học</i>";
$Fac = "Chủ đề (phân loại theo bộ môn)";
$Doubt = "Nếu chưa rõ cách đặt mã, xin hỏi";
$Program = "Chương trình Khoá học</a>.Nếu khoá học chưa có mã, hãy tạo mã có tính gợi ý và ngắn gọn";
$ForumLanguage = "tiếng Anh";
$NewCourse = "Khoá học mới";
$AddNewCourse = "Thêm khoá học mới";
$OtherProperties = "Các tính chất khác";
$SysId = "Mã hệ thống";
$ScoreShow = "Hiển thị điểm";
$Visibility = "Hiển thị";
$VersionDb = "Version de la base de donnée lors de l'archivage";
$Expire = "Hết hạn";
$ChoseFile = "Chọn file";
$FtpFileTips = "Si le fichier est sur un ordinateur tiers et accessible par ftp";
$HttpFileTips = "Si le fichier est sur un ordinateur tiers et accessible par http";
$LocalFileTips = "Si le fichier est sur l'espace de stockage des cours de ce campus";
$PostFileTips = "Si le fichier est sur  votre ordinateur";
$TitleNotification = "Từ lần ghé thăm trước";
$LinkName = "Tên liên kết";
$LinkAdd = "Đã tạo liên kết mới";
$LinkMod = "Đã sửa liên kết";
$LinkDel = "Đã xoá liên kết";
$GiveURL = "Xin nhập địa chỉ URL";
$ListDeleted = "Danh sách đã bị xoá";
$AddLink = "Thêm liên kết mới";
$DelList = "Xoá toàn bộ danh sách";
$ModifyLink = "Sửa liên kết";
$StatDB = "Tracking DB.  Useful only if several DB";
$EnableTracking = "Enable Tracking";
$InstituteShortName = "Organisation short name";
$WarningResponsible = "Use this script only after backup. Claroline team is not responsible if you lost or corrupted data";
$EG = "e. g.";
$DBHost = "Database Host";
$DBLogin = "Database Username";
$DBPassword = "Database Password";
$MainDB = "Main claroline DB";
$AllFieldsRequired = "all fields required";
$PrintVers = "Printable version";
$LocalPath = "Corresponding local path";
$AdminEmail = "Administrator email";
$AdminName = "Administrator name";
$AdminSurname = "Administrator surname";
$AdminLogin = "Administrator login";
$AdminPass = "Administrator password";
$EducationManager = "Education manager";
$CampusName = "Your campus' name";
$DBSettingIntro = "\t\t\t\tInstall script will create claroline main DB. Please note that Claroline \t\t\t\twill need to create many DBs (unless you select option \"One\" below). If you are allowed only one \t\t\t\tDB for your website by your Hosting Services, Claroline will not work.";
$Step3 = "Step 3 of 6";
$Step4 = "Step 4 of 6";
$Step5 = "Step 5 of 6";
$Step6 = "Step 6 of 6";
$CfgSetting = "Config settings";
$DBSetting = "MySQL database settings";
$MainLang = "Main language";
$Licence = "License";
$LastCheck = "Last check before install";
$DbPrefixForm = "MySQL Prefix";
$DbPrefixCom = "Leave empty if not requested";
$EncryptUserPass = "Encrypt user passwords in database";
$SingleDb = "Use one or several DB for Claroline";
$AllowSelfReg = "Allow self-registration";
$Recommended = "(recommended)";
$Titular = "Danh nghĩa";
$AdministrationTools = "Các công cụ quản trị";
$Added = "Đã thêm mới";
$Deleted = "Đã xóa";
$Keeped = "Giữ lại";
$HideAndSubscribeClosed = "Ẩn / Đóng";
$HideAndSubscribeOpen = "Ẩn / Mở";
$ShowAndSubscribeOpen = "Hiện / Mở";
$ShowAndSubscribeClosed = "Hiện / Đóng";
$AdminThisUser = "Trở về trang người dùng";
$Manage = "Quản lý chuyên môn";
$WCAGGoMenu = "Chuyển đến thực đơn";
$WCAGGoContent = "Chuyển đến nội dung";
$AdminBy = "Quản trị bởi";
$Statistiques = "Thống kê";
$Used = "used";
$Exist = "Thoát ra";
$DisplayTeacherInCourselistTitle = "Tên giáo viên trên tiêu đề khóa học";
$DisplayTeacherInCourselistComment = "Tên Giáo viên trong chú thích danh sách khóa học";
$AddAdminInApache = "Thêm người quản trịr";
$AddFaculties = "Thêm bộ môn";
$SearchACourse = "Tìm giáo trình";
$SearchAUser = "Tìm người dùng";
$TechnicalTools = "Quản trị kỹ thuật";
$Config = "Cầu hình hệ thống";
$LogIdentLogoutComplete = "Danh mục Login (mở rộng)";
$NomOutilTodo = "Quản lí danh mục cần làm";
$NomPageAdmin = "Quản trị";
$SysInfo = "Thông tin về hệ thống";
$DiffTranslation = "So sánh các bản dịch ngôn ngữ";
$StatOf = "Thống kê về";
$LogIdentLogout = "Danh mục đăng nhập";
$ServerStatus = "Trạng thái MySQL server :";
$DataBase = "Database";
$Run = "work";
$Client = "MySql Client";
$Server = "MySql Server";
$titulary = "Owner";
$UpgradeBase = "nâng cấp cơ sở dữ liệu";
$ErrorsFound = "lỗi tìm thấy";
$Maintenance = "Bảo trì";
$Upgrade = "Nâng cấp";
$Website = "Claroline website";
$Documentation = "Tài liệu";
$Contribute = "Tài trợ";
$AnnEmpty = "Danh mục các thông báo đã được xoá";
$AnnouncementModified = "Thông báo đã chỉnh sửa";
$AnnouncementAdded = "ThĂ´ng bĂ¡o má»›i Ä‘Ă£ Ä‘Æ°á»£c Ä‘Æ°a vĂ o";
$AnnouncementDeleted = "Thông báo đã bị xoá";
$AnnouncementPublishedOn = "NgĂ y";
$AddAnnouncement = "Tạo thông báo mới";
$AnnouncementDeleteAll = "Xoá tất cả thông báo";
$professorMessage = "Thư của giáo viên";
$EmailSent = " và đã gữi cho các học sinh có đăng ký";
$EmailOption = "Gá»­i thĂ´ng bĂ¡o nĂ y cho cĂ¡c há»";
$On = "On";
$RegUser = "người dùng đã đăng ký";
$Unvalid = "không có địa chỉ email hoặc địa chỉ email không hợp lệ";
$ModifAnn = "Sửa thông báo này";
$LineNumber = "Số dòng";
$LineOrLines = "dòng";
$AddNewHeading = "Thêm tiêu đề mới";
$CourseAdministratorOnly = "Chỉ dành cho người quản trị";
$DefineHeadings = "Xác định các tiêu đề";
$BackToUsersList = "Trở về danh sách người dùng";
$CourseManager = "Quản lý khoá học";
$NewGroupCreate = "Tạo nhóm mới";
$GroupCreation = "Nhóm vừa tạo";
$NewGroups = "Nhóm mới";
$Max = "tối đa 12 kí tự, v.d. <i>TOAN12</i>";
$GroupPlacesThis = "seats (optional)";
$GroupsAdded = "đã thêm nhóm mới";
$GroupDel = "đã xóa";
$ExistingGroups = "nhóm";
$Registered = "đã đăng kí";
$GroupAllowStudentRegistration = "nhóm cho phép sinh viên đăng kí";
$GroupTools = "công cụ";
$GroupDocument = "tài liệu";
$GroupPropertiesModified = "đặc tính về nhóm đã được chỉnh sửa";
$GroupName = "Tên nhóm";
$GroupDescription = "Miêu tả về nhóm";
$GroupMembers = "Thành viên của nhóm";
$EditGroup = "Chỉnh sửa nhóm";
$GroupSettingsModified = "Những điều chỉnh về nhóm đã được thay đổi";
$GroupTooMuchMembers = "Number proposed exceeds max. that you allowed (you can modify it below). \t\t\t\tGroup composition has not been modified";
$GroupTutor = "Tutor";
$GroupNoTutor = "(none)";
$GroupNone = "(none)";
$GroupNoneMasc = "(none)";
$AddTutors = "Danh sách người dùng quản trị";
$MyGroup = "Nhóm của tôi";
$OneMyGroups = "my supervision";
$GroupSelfRegistration = "Đăng ký";
$GroupSelfRegInf = "đăng ký";
$RegIntoGroup = "Thêm tôi vào nhóm này";
$GroupNowMember = "Bạn hiện là thành viên của nhóm này";
$Private = "Truy cập chỉ với <a href=../user/user.php>danh sách user</a>)";
$Public = "Truy cập không cần đăng nhập";
$PropModify = "Thay đổi";
$State = "Trạng thái";
$GroupFilledGroups = "Groups have been filled (or completed) by students present in the 'Users' list.";
$Subscribed = "Người đăng kí khóa học này";
$StudentsNotInThisGroups = "Bạn không thuộc nhóm này";
$QtyOfUserCanSubscribe_PartBeforeNumber = "A user can be member of maximum";
$QtyOfUserCanSubscribe_PartAfterNumber = "A groups";
$StudentRegAllowed = "Cho phép sinh viên đăng kí vào nhóm";
$Exercice = "Bài tập";
$NoEx = "Chưa có bài tập";
$NewEx = "Bài tập mới";
$Questions = "Câu hỏi";
$Answers = "Trả lời";
$True = "True";
$Answer = "Trả lời";
$ExerciseType = "Loại bài tập";
$ExerciseName = " Tên bài tập trắc nghiệm";
$ExerciseDescription = "Ghi chú về bài tập";
$SimpleExercise = "Trên một trang";
$SequentialExercise = "Mỗi câu hỏi một trang (liên tiếp)";
$RandomQuestions = "Câu hỏi ngẫu nhiên";
$GiveExerciseName = "Xin ghi tên bài tập";
$NoAnswer = "Chưa có câu trả lời";
$GoBackToQuestionPool = "Trở về phần lập câu hỏi";
$GoBackToQuestionList = "Trở về phần danh mục câu hỏi";
$QuestionAnswers = "Trả lời cho câu hỏi";
$UsedInSeveralExercises = "Coi chừng! Câu hỏi này dùng trong nhiều bài tập khác nhau.";
$ModifyInAllExercises = "trong tất cả bài tập";
$ModifyInThisExercise = "chỉ trong bài tập hiện hành";
$AnswerType = "Kiểu trả lời";
$MultipleSelect = "Nhiều lựa chọn đúng";
$FillBlanks = "Điền vào chỗ trống";
$Matching = "Ghép cột";
$ReplacePicture = "Thay thế hình";
$DeletePicture = "Xoá hình";
$QuestionDescription = "Ghi chú tuỳ chọnt";
$GiveQuestion = "Hãy đặt câu hỏi";
$WeightingForEachBlank = "Xin cho điểm vào mỗi ô trống";
$UseTagForBlank = "dùng ngoặc vuông [...] để định nghĩa các chỗ cần điền";
$QuestionWeighting = "Điểm";
$MoreAnswers = "+answ";
$LessAnswers = "-answ";
$MoreElements = "+elem";
$LessElements = "-elem";
$TypeTextBelow = "Nhập vào phần sau đây";
$DefaultTextInBlanks = "[Biên Hoà] là thành phố thuộc tỉnh [Đồng Nai].";
$DefaultMatchingOptA = "giàu có";
$DefaultMatchingOptB = "đẹp";
$DefaultMakeCorrespond1 = "Bố của bạn";
$DefaultMakeCorrespond2 = "Mẹ của bạn";
$DefineOptions = "Định nghĩa các chọn lựa";
$MakeCorrespond = "Thiết lập tương ứng";
$FillLists = "Hãy điền vào hai danh sách dưới đây";
$GiveText = "Gõ vào";
$DefineBlanks = "Phải có í nhất 1 từ trong ngoặc [...]";
$GiveAnswers = "Phải nêu trả lời cho câu hỏi";
$ChooseGoodAnswer = "Chọn câu trả lời đúng";
$ChooseGoodAnswers = "Chọn một hay nhiều câu trả lời đúng";
$QuestionList = "Danh mục câu hỏi trong bài tập";
$GetExistingQuestion = "Lấy câu hỏi từ một bài tập khác";
$QuestionPool = "Kho câu hỏi";
$OrphanQuestions = "Câu hỏi mồ côi";
$NoQuestion = "Chưa có câu hỏi";
$AllExercises = "Tất cả bài tập";
$GoBackToEx = "Về bài tập";
$Reuse = "Dùng lại";
$ExerciseManagement = "Quản lý bài tập";
$QuestionManagement = "Quản lý câu hỏi/câu trả lờit";
$QuestionNotFound = "Không tìm thấy câu hỏi";
$ExerciseNotFound = "Không tìm thấy bài tập";
$AlreadyAnswered = "Bạn đã trả lời câu này rồi";
$ElementList = "Liệt kê";
$CorrespondsTo = "Tương ứng với";
$ExpectedChoice = "Đáp án";
$YourTotalScore = "Tổng điểm của bạn là";
$Remove = "Xoá";
$Enter = "Trở lại danh mục khoá học";
$Description = "Mô tả tóm tắt";
$Links = "Liên kết";
$Works = "Bài làm";
$Forums = "Diễn đàn";
$CreateDir = "Tạo thư mục";
$Name = "Tên";
$Comment = "Ghi chú";
$Visible = "Hiển thị/không hiển thị";
$NewDir = "Tên thư mục mới";
$DirCr = "Thư mục đã tạo";
$Group = "Group";
$Edit = "Sửa";
$GroupForum = "Diễn đàn";
$Language = "Ngôn ngữ";
$Assignments = "Bài tập";
$Forum = "Diễn đàn";
$Code = "Mã khoá học";
$Up = "Up";
$UniqueSelect = "Một lựa chọn đúng";
$CourseCreate = "Tạo một khoá học";
$Todo = "Kiến nghị";
$UserName = "Tên đăng nhập";
$Dear = "Dear";
$iso639_2_code = "en";
$iso639_1_code = "eng";
$left_font_family = "verdana, helvetica, arial, geneva, sans-serif";
$right_font_family = "helvetica, arial, geneva, sans-serif";
$number_thousands_separator = ",";
$number_decimal_separator = ".";
$dateFormatShort = "%d-%m-%y";
$dateFormatLong = "%A, %d-%m-%Y";
$dateTimeFormatLong = "%d - %m, %Y - %I:%M %p";
$timeNoSecFormat = "%I:%M %p";
$Yes = "Có";
$No = "Không";
$Next = "Tiếp";
$Allowed = "Được phép";
$BackHome = "Trở về trang chính của";
$Propositions = "Đề xuất cải tiến về";
$Maj = "Cập nhật";
$Modify = "Thay đổi";
$Move = "Di chuyển";
$Help = "Trợ giúp";
$Ok = "Ok";
$Add = "Thêm";
$AddIntro = "Lời giới thiệu";
$BackList = "Về danh mục";
$Text = "Văn bản";
$Empty = "Bạn còn chừa trống ô.<br>Bấm nút <b>Back</b>để làm lại.<br>";
$ConfirmYourChoice = "Hãy xác nhận lại lựa chọn của bạn";
$And = "và";
$Choice = "Bạn đã chọn";
$Finish = "Kết thúc";
$Cancel = "Hủy";
$NotAllowed = "Không được chấp nhận";
$Manager = "Quản trị Website trắc nghiệm trực tuyến";
$Optional = "Tuỳ chọn";
$NextPage = "Trang kế tiếp";
$PreviousPage = "Trang trước";
$Use = "Sử dụng";
$Total = "Tổng cộng";
$Take = "cần";
$One = "Một";
$Several = "Nhiều";
$Notice = "Ghi chú";
$Date = "Ngày";
$Among = "trong số";
$ModifyProfile = "Sửa thông tin cá nhân";
$MyStats = "Xem thống kê";
$Logout = "Thoát";
$CourseProgram = "Mô tả tóm tắt";
$ThisCourseDescriptionIsEmpty = "Giáo trình này không có phần diễn giải";
$QuestionPlan = "Câu hỏi với giáo viên";
$NewBloc = "Khác";
$InvalidId = "<small>Không đăng nhập được.Nếu bạn chưa phải thành viên, hãy <a href=claroline/auth/inscription.php>đăng ký ở đây</a></small></font color>";
$Pass = "Password";
$Advises = "Advises";
$SupportForum = "Diễn đàn hỗ trợ";
$BackToHomePage = "Trở về trang chủ";
$LostPassword = "Quên mật khẩu";
$YourPasswordHasBeenEmailed = "Mật khẩu của bạn đã được gửi tới email";
$EnterEmailAndWeWillSendYouYourPassword = "Nhập địa chỉ email bạn đã đăng kí, chúng tôi sẽ gửi mật khẩu cho bạn thông qua email.";
$Action = "Hành động";
$Preserved = "Đã lưu";
$ConfirmUnsubscribe = "Xác nhận đăng kí";
$See = "Go to";
$LastVisits = "Lần ghé thăm gần đây nhất";
$IfYouWantToAddManyUsers = "Nếu bạn muốn thêm mới nhiều người dùng cùng 1 lúc, xin liên hệ với người quản trị‹";
$PassTooEasy = "Mật khẩu không đủ mạnh. Nên sử dụng mật khẩu khác";
$AddedToCourse = "Đã đăng kí theo khóa học";
$UserAlreadyRegistered = "Trùng họ / tên với người dùng hiện có";
$BackUser = "Trở về danh sách người dùng";
$UserOneByOneExplanation = "Người dùng sẽ nhận được emmail thông báo login và  password";
$GiveTutor = "gán quyền giáo viên";
$RemoveRight = "Xóa quyền này";
$GiveAdmin = "Gán quyền admin";
$UserNumber = "số lượng";
$DownloadUserList = "Danh sách Upload";
$UserAddExplanation = "every line of file to send will necessarily an only\t\tinclude 5 fields: <b>Last NameÂ Â Â First NameÂ Â Â \t\tLoginÂ Â Â PasswordÂ \t\tÂ Â Email</b> separated by tabs and in this order.\t\tUsers will recieve email confirmation with login/password.";
$UserMany = "Import users list through textfile";
$OneByOne = "Sau khi đăng kí";
$AddHereSomeCourses = "<font size=2 face='Arial, Helvetica'><big>Hiệu chỉnh danh sách khóa học</big><br><br>Đánh dấu vào khóa học bạn chọn<br>Ngược lại bỏ đánh dấu nếu không chọn<br> Sau đó bấm nút OK tại cuối danh sách";
$AddAU = "Thêm user";
$AddedU = "Đã được thêm. Sẽ gửi thông báo đến email";
$TheU = "Người dùng";
$RegYou = "Đã đăng kí cho bạn trong khóa học này";
$OneResp = "Một trong số những quản trị viên của khóa học";
$ProfileReg = "Hồ sơ của bạn đã được lưu lại";
$EmailWrong = "Email này không tồn tại hoặc chưa 1 vài kí tự không hợp lệ";
$UserTaken = "Tên đăng nhập này đã được sử dụng";
$Fields = "Bạn chưa điền 1 số thông tin bắt buộc";
$Again = "Thử lại lần nữa";
$PassTwo = "Bạn xác nhận sai mật khẩu";
$ModifProfile = "Sửa hồ sơ";
$IsReg = "Sửa đổi của bạn được bảo đảm";
$NowGoCreateYourCourse = "Hiện tại bạn đã có thể tạo khóa học";
$NowGoChooseYourCourses = "Bây giờ bạn đã có thể lựa chọn khóa học trong danh sách khóa học";
$PersonalSettings = "<p>Thông tin cá nhân của bạn đã được đảm bảo và 1 email đã được gửi để giúp bạn ghi nhớ tên đăng nhập và mật khẩu</p>";
$Problem = "Trong trường hợp có lỗi xảy ra, hãy liên hệ với chúng tôi";
$Is = "is";
$Subscribe = "Đăng ký";
$Address = "địa chỉ của";
$YourReg = "Your registration on";
$UserFree = "Tên đăng nhập đã có người sử dụng. Sử dụng nút quay lại của trình duyệt và chọn 1 kết quả khac";
$EmptyFields = "Bạn để trống 1 số thông tin. Sử dụng nút quay lại của trình duyệt và thử lại lần nữa.";
$PassTwice = "Bạn vừa xác nhận sai mật khẩu. Sử dụng nút quay lại của trình duyệt và thử lại lần nữa";
$RegAdmin = "Tạo website cho khóa học(GV)";
$RegStudent = "Tham gia các khóa học(HS)";
$Confirmation = "Nhập lại mật khẩu";
$Surname = "tên";
$Registration = "Đăng kí";
$YourAccountParam = "điền tên đăng nhập và mật khẩu của bạn";
$LoginRequest = "Cần phải đăng nhập";
$AdminOfCourse = "admin";
$SimpleUserOfCourse = "normal";
$IsTutor = "giáo viên";
$ParamInTheCourse = "Trạng thái";
$HaveNoCourse = "không có khóa học";
$UserProfileReg = "updated";
$Courses4User = "Các khóa học dành cho người dùng";
$CoursesByUser = "Các khóa học được tạo bởi người dùng";
$InLnk = "Tạm ngưng hoạt động";
$DelLk = "Bạn muốn xoá liên kết này?";
$NameOfTheLink = "Tên của liên kết";
$ToolName = "Thống kê";
$TrackingDisabled = "Theo dõi được ngừng hoạt động bởi quản trị hệ thống.";
$CourseStats = "Trạng thái khóa học";
$ToolsAccess = "Công cụ truy cập";
$LinksAccess = "Liên kết";
$DocumentsAccess = "Truy cập tài liệu";
$LinksDetails = "Liên kết truy cập bởi học sinh";
$WorksDetails = "Bài làm do học sinh tải lên";
$LoginsDetails = "Bấm vào tên tháng để xem chi tiết";
$DocumentsDetails = "Tài liệu học sinh download về";
$ExercicesDetails = "điểm số các bài tập";
$BackToList = "Trở về danh sách người dùng";
$StatsOfCourse = "Thống kê khóa học";
$StatsOfUser = "Thống kê người dùng";
$StatsOfCampus = "Thống kê chung";
$CountUsers = "Số lượng người dùng";
$CountToolAccess = "Tổng số lượt truy cập khóa học";
$LoginsTitleMonthColumn = "Tháng";
$LoginsTitleCountColumn = "Số lượt đăng nhập";
$ToolTitleToolnameColumn = "Tên công cụ";
$ToolTitleUsersColumn = "Số lượt truy cập của user";
$ToolTitleCountColumn = "Tổng số lượt truy cập";
$LinksTitleLinkColumn = "Liên kết";
$LinksTitleUsersColumn = "Số lượt truy cập của user";
$LinksTitleCountColumn = "Tổng số lượt truy cập";
$ExercicesTitleExerciceColumn = "Bài tập";
$ExercicesTitleScoreColumn = "điểm";
$DocumentsTitleDocumentColumn = "Tài liệu";
$DocumentsTitleUsersColumn = "Số lượng Downloads";
$DocumentsTitleCountColumn = "Tổng số lượng Downloads";
$WorkTitle = "Tiêu đề";
$WorkAuthors = "Người gửi";
$WorkDescription = "Ghi chú";
$informationsAbout = "Theo dõi";
$NoEmail = "Chưa có địa chỉ email";
$NoResult = "Chưa có kết quả";
$Hits = "Hits";
$LittleHour = "h.";
$Last31days = "Trong 31 ngày gần đây";
$Last7days = "Trong 7 ngày gần đây";
$ThisDay = "Hôm nay";
$Logins = "Đăng nhập";
$LoginsExplaination = "đây là danh sách các công cụ bạn đã sử dụng trong lần truy cập gần đây nhất.";
$ExercicesResults = "Kết quả các bài tập";
$At = "tại";
$StartDate = "Bắt đầu";
$Result = "Kết quả";
$Attempt = "Lượt thi";
$Username = "Họ và tên";
$LoginTitleDateColumn = "Ngày";
$LoginTitleCountColumn = "Lượt ghé thăm";
$LoginsAndAccessTools = "Công cụ đăng nhập và truy cập";
$WorkUploads = "Các  bài đã upload";
$ErrorUserNotInGroup = "Không có trong nhóm";
$ListStudents = "Danh sách học sinh trong nhóm";
$PeriodHour = "Giờ";
$PeriodDay = "Ngày";
$PeriodWeek = "Tuầnn";
$PeriodMonth = "Tháng";
$PeriodYear = "Năm";
$NextDay = "Ngày kế tiếp";
$PreviousDay = "Ngày trước";
$NextWeek = "Tuần kế tiếp";
$PreviousWeek = "Tuần trước";
$NextMonth = "Tháng kê tiếp";
$PreviousMonth = "Tháng trước";
$NextYear = "Năm kế tiếp";
$PreviousYear = "Năm trước";
$ViewToolList = "Xem danh sách tất cả công cụ";
$ToolList = "Danh sách tất cả công cụ";
$PeriodToDisplay = "Chu kì";
$DetailView = "Xem thông tin";
$BredCrumpGroups = "Nhóm";
$BredCrumpGroupSpace = "Không gian của nhóm";
$BredCrumpUsers = "Người dùng";
$AdminToolName = "Admin Stats";
$PlatformStats = "Trạng thái hệ thống";
$StatsDatabase = "Trạng thái cơ sở dữ liệu";
$PlatformAccess = "Truy cập hệ thống";
$PlatformCoursesAccess = "Truy cập khóa học";
$PlatformToolAccess = "Truy cập công cụ";
$HardAndSoftUsed = "Các quốc gia cung cấp trình duyệt os Referers";
$StrangeCases = "Trường hợp lạ";
$StatsDatabaseLink = "Nhấn vào đây";
$CountCours = "Số khóa học";
$CountCourseByFaculte = "Số lượng khóa học bộ môn";
$CountCourseByLanguage = "Số lượng khóa học ngôn ngữ";
$CountCourseByVisibility = "Số lượng các khóa học theo mức độ hiển thị";
$CountUsersByCourse = "Số lượng người dùng theo khóa học";
$CountUsersByFaculte = "Số lượng người dùng theo tùy chọn";
$CountUsersByStatus = "Số lượng người sử dụng theo tình trạng";
$Access = "Truy cập";
$Countries = "Quốc gia";
$Providers = "Người cung cấp";
$OS = "OS";
$Browsers = "Các trình duyệt";
$Referers = "Giới thiệu";
$AccessExplain = "(When an user open the index of the campus)";
$TotalPlatformAccess = "Tổng số lượt truy cập hệ thống";
$TotalPlatformLogin = "Tổng số đăng nhập";
$MultipleLogins = "Accounts with same <i>Username</i>";
$MultipleUsernameAndPassword = "Accounts with same <i>Username</i> AND same <i>Password</i>";
$MultipleEmails = "Accounts with same <i>Email</i>";
$CourseWithoutProf = "Các khóa học không có giáo viên";
$CourseWithoutAccess = "Các khóa học không được sử dụng";
$LoginWithoutAccess = "Thông tin đăng nhập không được sử dụng";
$AllRight = "Không có trường hợp lạ ở đây";
$Defcon = "Ooops, stranges cases detected !!";
$NULLValue = "Không có giá trị";
$TrafficDetails = "Traffic Details";
$GroupSpaceLink = "Khu vực nhóm";
$HFor = "Hỗ trợ diễn đàn";
$ForContent = "Diễn đàn là nơi thảo luận không đồng bộ. Nơi mà email có thể trao đối, đối thoại công khai.</p><p>Về mặt kĩ thuật, sinh viên chỉ cần có trình duyệt web là có thể sử dụng diễn đàn</P><p>Để liên lạc với tổ chức diễn đàn, nhấp vào 'Administer'. Các cuộc thảo luận được tổ chức đồng bộ và được bố trí như sau:</p><p><b>Danh mục > Diễn đàn > Chủ đề > Trả lời</b></p>Để cấu trúc được các cuộc thảo luận của sinh viên, it is necessary to organise catgories and forums beforehand, leaving the creation of topics and answers to them. By default, the claroline forum contains only the category 'Public', a sample forum ans a sample topic.</p><p>The first thing you should do is deleting the sample topic and modify the first forum's name. Then, you can create, in the 'public' category, other forums, by groups or by themes, to fit your learning scenario requirements.</p><p>Don't mix Categories and forums, and don't forget that an empty category (without forums) does not appear on the student view.</p><p>The description of a forum can be the list of its members, the definition of a goal, a task, a theme...";
$HHome = "Hỗ trợ trang chủ";
$HomeContent = "để thuận tiện hơn, những công cụ claroline chứa các mục mặc định. Có 1 ví dụ nhỏ trong tất cả công cụ để giúp bạn nắm thật nhanh nó hoạt động như thế nào.Bạn có thể sửa đổi hoặc xóa nó.</p><p>Ví dụ, trên trang chủ website khóa học của bạn, có 1 dòng giới thiệu nhỏ : ' Đây là phần giới thiệu cho khóa học của bạn. Để thay đổi nói, nhấp vào modify ở phía dưới'. Sau khi nhấp vào modify, thay đổi nó và OK. Nó thật đơn giản. Every tool has the same logic: add, delete, modify, which is the logic of dynamic websites.</p><p>When you first create your website, most of the tools are active. Here again, it is up to you to deactivate the ones you don't need. You just have to click on 'deactivate'. Then it goes down to the grey section of your homepage an becomes invisible to your students.  However, you can reactivate it whenever you want, making it visible to the students once more.</p><p>You can add your own pages to your Home Page. These pages must be HTML pages (which can be created by any Word Processor or Web Composer). Use 'Upload page and link to Homepage' to send your page to the server. The standard header of your website will be automatically merged with your new document, so that you just need to concentrate on the content. If you want to link from your Home towards existaing websites or pages existing anywhere on the web (even inside your own site), use 'Add link on Homepage' The pages you have added to the Home page can deactivated then deleted, where the standard tools can be deactivated, but not deleted.</p><p>Once your course website is ready, go to 'Modify course info' and decide what level of confidentiality you want. By default, your course is hidden (because you work on it).</p>";
$HClar = "Hỗ trợ khởi đầu";
$HDoc = "Hỗ trợ tài liệu";
$HUser = "Hỗ trợ người dùng";
$HExercise = "Hỗ trợ bài tập";
$Height = "Chiều cao";
$ResizingComment = "thay đổi kích thước bức ảnh thep pixels";
$Width = "Chiều rộng";
$Resizing = "thay đổi kích thước";
$NoResizingComment = "Hiển thị toàn bộ những bức ảnh với kích thước thật của nó. Con lăn sẽ tự động xuất hiện nếu kích thước bức ảnh lớn hơn kích thước màn hình máy tính của bạn.";
$ShowThumbnails = "Hiển thị hình nhỏ";
$SetSlideshowOptions = "Cài đặt các lựa chọn cho slideshow";
$SlideshowOptions = "Những lựa chọn của Slide";
$NoResizing = "Mặc định";
$SlideShow = "Slideshow";
$ViewSlideshow = "Xem slide";
$ResourceAdded = "Tài nguyên đã được thêm vào.Bây giờ bạn có thể quay lại hoặc thêm tài nguyên mới";
$LearningPath = "Quá trình học tập";
$LevelUp = "level up";
$AddIt = "Thêm nó";
$MainCategory = "Danh mục chính";
$AddToLinks = "Thêm liên kết vào khóa học";
$DontAdd = "Không thêm";
$ResourcesAdded = "Thêm tài nguyên mới";
$ExternalResources = "Tài nguyên bên ngoài";
$CourseResources = "Tài nguyên khóa học";
$ExternalLink = "Liên kết ngoài";
$DropboxAdd = "Thêm Dropbox cho chương này";
$AddAssignmentPage = "Thêm các trang phân tải lên chương này.";
$SendWork = "Upload bài";
$TooBig = "Bạn không chọn tệp nào, hoặc là kích thước tệp quá lớn";
$DocModif = "tên bài đã sửa";
$DocAdd = "Bài đã nộp";
$DocDel = "Bài đã xoá";
$TitleWork = "Tên bài";
$Authors = "Người gửi";
$WorkDelete = "Xoá";
$WorkModify = "Sửa";
$WorkConfirmDelete = "Bạn có chắc muốn xoá bài nàyr";
$AllFiles = "Tất cả file";
$UserContent = "<b>Roles</b><p>Roles have no computer related function. They do not give rights on operating the system. They just indicate to Humans, who is who. You can modify them by clicking on 'modify' under 'role', then typing whatever you want: professor, assistant, student, visitor, expert...</P><hr><b>Admin rights</b><p>Admin rights, on the other hand, correspond to the technical authorisation to modify the content and organisation of the course website. For the moment, you can only choose between giving all the admin rights and giving none of them.</P><p>To allow an assistant, for instance, to co-admin the site, you need to register him in the course or be sure he is already registerd, then click on 'modify' under 'admin rights', then click 'all', then 'Ok'.</P><hr><b>Co-chairmen</b><p>To mention in the header of the course website the name of a co-chairmen, use the tool 'Modify course information' (orange tools). This modification does not register your co-chairmen as a user of the course. The field 'Professors' is completely independant of the Users list.</p><hr><b>Add a user</b><p>To add a user for your course, fill the fields and validate. The personwill receive an email telling him/her you have registered him/her and tellinghim/her or reminding him/her his/her login and  password.</p>";
$GroupContent = "<p><b>Introduction</b></p><p>This tool allows to create and manage work groups.At creation (Create groups), groups are emtpy. There aremany ways to fill them:<ul><li>automatically ('Fill groups'),</li><li>manually ('Edit'),</li><li>self-registration by students diants (Groups settings: 'Self registration allowed...').</li></ul>These three ways can be combined. You can, for instance, ask students to self-register first.Then discover that some of them didn't and decide then to fill groups automatically inorder to complete them. You can also edit each group to compose membership one studentat a time after or before self-registration and/or automatical filling.</p><p>Groups filling, whether automatical or manual, works only if there are already studentsregistered in the course (don't mix registration to the course with registration into groups).Students list is visible in <b>Users</b> tool. </p><hr noshade size=1><p><b>Create groups</b></p><p>To create new groups, click on 'Create new group(s)' and determine number of groups tocreate. Maximum number of members is optional but we suggest to chose one. If you leave max. fieldunchanged, groups size maximum will be infinite.</p><hr noshade size=1><p><b>Group settings</b></p><p>You can determine Group settings globally (for all groups).<b>Students are allowed to self-register in groups</b>:<p>You create empty groups, students self-register.If you have defined a maximum number, full groups do not accept new members.This method is good for teachers who do not know students list whencreating groups.</p><b>Outils</b>:</p><p>Every group possesses either a forum (private or public) or a Documents area(a shared file manager) or (most frequent) both.</p><hr noshade size=1><p><b>Manual edit</b></p><p>Once groups created (Create groups), you see at bottom of page, a list of groupswith a series of informations and functions<ul><li><b>Edit</b> to modify manually Group name, description, tutor,members list.</li><li><b>Delete</b> deletes a group.</li></ul><hr noshade size=1>";
$ExerciseContent = "<p>The exercise tool allows you to create exercises that will contains as many questions as you like.<br><br>There are various types of answers available for the creation of your questions :<br><br><ul>  <li>Multiple choice (Unique answer)</li>  <li>Multiple choice (multiple answers)</li>  <li>Matching</li>  <li>Fill in the blanks</li></ul>An exercise gathers a certain number of questions under a common theme.</p><hr><b>Exercise creation</b><p>In order to create an exercise, click on the link \"New exercise\".<br><br>Type the exercise name, as well as an optional description of it.<br><br>You can also choose between 2 exercise types :<br><br><ul>  <li>Questions on an unique page</li>  <li>One question per page (sequential)</li></ul>and tell if you want or not questions to be randomly sorted at the time of the exercise running.<br><br>Then, save your exercise. You will go to to the question administration for this exercise.</p><hr><b>Question adding</b><p>You can now add a question into the exercise previously created. The description is optional, as well as the picture that you have the possibility of linking to your question.</p><hr><b>Multiple choice</b><p>This is the famous MAQ (multiple answer question) / MCQ (multiple choice question).<br><br>In order to create a MAQ / MCQ :<br><br><ul>  <li>Define answers for your question. You can add or delete an answer by clicking on the right button</li>  <li>Check via the left box the correct answer(s)</li>  <li>Add an optional comment. This comment won't be seen by the student till this one has replied to the question</li>  <li>Give a weighting to each answer. The weighting can be any positive or negatif integer, or zero</li>  <li>Save your answers</li></ul></p><hr><b>Fill in the blanks</b><p>This allows you to create a text with gaps. The aim is to let student find words that you have removed from the text.<br><br>To remove a word from the text, and so to create a blank, put this word between brackets [like this].<br><br>Once the text has been typed and blanks defined, you can add a comment that will be seen by the student when it replies to the question.<br><br>Save your text, and you will enter the next step that will allow you to give a weighting to each blank. For example, if the question worths 10 points and you have 5 blanks, you can give a weighting of 2 points to each blank.</p><hr><b>Matching</b><p>This answer type can be chosen so as to create a question where the student will have to connect elements from an unit U1 with elements from an unit U2.<br><br>It can also be used to ask students to sort elements in a certain order.<br><br>First define the options among which the student will be able to choose the good answer. Then, define the questions which will have to be linked to one of the options previously defined. Finally, connect via the drop-down menu elements from the first unit with those of the second one.<br><br>Notice : Several elements from the first unit can point to the same element in the second unit.<br><br>Give a weighting to each correct matching, and save your answer.</p><hr><b>Exercise modification</b><p>In order to modify an exercise, the principle is the same as for the creation. Just click on the picture <img src=\"../img/edit.gif\" border=\"0\" align=\"absmiddle\"> beside the exercise to modify, and follow instructions above.</p><hr><b>Exercise deleting</b><p>In order to delete an exercise, click on the picture <img src=\"../img/delete.gif\" border=\"0\" align=\"absmiddle\"> beside the exercise to delete.</p><hr><b>Exercise enabling</b><p>So as for an exercise to be used, you have to enable it by clicking on the picture <img src=\"../img/invisible.gif\" border=\"0\" align=\"absmiddle\"> beside the exercise to enable.</p><hr><b>Exercise running</b><p>You can test your exercise by clicking on its name in the exercise list.</p><hr><b>Random exercises</b><p>At the time of an exercise creation / modification, you can tell if you want questions to be drawn in a random order among all questions of the exercise.<br><br>That means that, by enabling this option, questions will be drawn in a different order each time students will run the exercise.<br><br>If you have got a big number of questions, you can also choose to randomly draw only X questions among all questions available in that exercise.</p><hr><b>Question pool</b><p>When you delete an exercise, questions of its own are not removed from the data base, and can be reused into a new exercise, via the question pool.<br><br>The question pool also allows to reuse a same questions into several exercises.<br><br>By default, all questions of your course are shown. You can show the questions related to an exercise, by chosing this one in the drop-down menu \"Filter\".<br><br>Orphan questions are questions that don't belong to any exercise.</p>";
$ClarContent3 = "</p><p><b>Educational Theory</b><p>For the professors, prepare a course on the internet is a question of Educational Theory too.";
$ClarContent4 = "is at your disposal to help you during the different steps of your teaching project evolution: from tool design to its integration in a clear and coherent strategy and objective evaluation of its impact on student learning.</p>";
$CampusHomepage = "Trang Chủ";
$PortalHomepageDefaultIntroduction = "<h1>Xin chào quản trị viên</h1>";
$HottestCourses = "Các khóa học phổ biến nhất";
$Dashboard = "Bảng điều khiển";
$MyAgenda = "Thời gian biểu";
$NbUsers = "Người dùng";
$ManageUsers = "Quản lý người dùng";
$Users = "Người dùng";
$Portal = "Cổng thông tin";

/** Users */ 
$AddUsers = "Thêm người dùng";
$AddAUser = "Thêm người dùng";
$ExportUsers = "Xuất danh sách người dùng";
$ExportUserListXMLCSV = "Xuất danh sách người dùng";
$EditUserListCSV = "Chỉnh sửa danh sách người dùng";
$BulkAnonymizeUsers = "Danh sách người dùng ẩn";
$ManageUserFields = "Hồ sơ";

/** Skills */ 
$Skills = "Kỹ năng";
$MyCertificates = "Chứng chỉ của tôi";
$MySkills = "Kĩ năng của tôi";
$ManageSkills = "Quản lý kỹ năng";
$SearchSkills = "Kỹ năng tìm kiếm";
$AddSkills = "Thêm kỹ năng";
$Profile = "Hồ sơ";
$Inbox = "Hộp thư đến";
$Compose = "Biên soạn";
$PendingInvitations = "Lời mời đang chờ xử lý";
$PersonalDataReport = "Dữ liệu cá nhân";
$EditInformation = "Sửa thông tin cá nhân";
$Learnpaths = "Danh mục môn học";
$AddSession = "Thêm một buổi đào tạo";
$SortMyCourses = "Sắp xếp các khóa học";
$HistoryTrainingSessions = "Lịch sử các khóa học";
$CourseCatalog = "Danh mục các môn học";
$LastVisitedCourse = "Khóa học được truy cập gần đây nhất";
$LastVisitedLp = "Lộ trình học tập được truy cập lần cuối";
$Agenda = "Thời gian biểu";
$SkillsWheel = "Skills wheel";
$SkillsImport = "Skills import";
$ManageSkillsLevels = "Quản lý cấp độ kỹ năng";
$ScoreEdit = "Xếp hạng kỹ năng";
$ScoringSystem = "Skills ranking";
$GradebookSkillsRanking = "Skills ranking";
$SkillsRanking = "Xếp hạng kỹ năng";
$SkillsAndGradebooks = "Kỹ năng và đánh giá";
$ImportSkillsListCSV = "Nhập kỹ năng từ tệp CSV";


/** PersonalDataPrivacy */ 
$PersonalDataPrivacy = "Bảo mật dữ liệu cá nhân";
$UserList = "Danh sách người dùng";
$InformationRightToBeForgottenLinkX = "You can find more information about the user's right to be forgotten through the following page: %s";

/** Courses */ 
$Courses = "Danh mục môn học";
$MyCourses = "Danh mục khoá học";
$ExportCourses = "Xuất khóa học";
$ImportCourses = "Nhập danh sách các khóa học";
$AssignCourses = "Chỉ định các khóa học";
$FillCourses = "Điền vào các khóa học";
$FollowedCourses = "Các khóa học đã theo dõi";
$BaseCourses = "Các khóa học cơ bản";
$RegistrationRoleFollowCourses = "Theo dõi các khóa học";
$RegistrationRoleTeachCourses = "Dạy các khóa học";
$ExploreMoreCourses = "Tìm hiểu các khóa học khác";
$SkillRelCourses = "Courses-Skills associations";
$RequiredCourses = "Khóa học yêu cầu";
$BuyCourses = "Mua khóa học";
$AssignedCourses = "Các khóa học được chỉ định";
$AddMeAsTeacherInCourses = "Add me as teacher in the imported courses.";
$ImportPDFIntroToCourses = "Nhập phần giới thiệu PDF vào các khóa học";
$SpecialCourses = "Các khóa học đặc biệt";
$TimeSpentByStudentsInCourses = "Thời gian dành cho sinh viên trong các khóa học";
$SortMyCourses = "Sắp xếp các khóa học";
$ViewOpenCourses = "Xem các khóa học công khai";
$AdminCourses = "Các khóa học";
$AdminManageVirtualCourses = "Quản lý các khóa học ảo";
$DisplayListVirtualCourses = "Hiển thị danh sách các khóa học ảo";
$NbCourses = "Các khóa học";
$NumberOfCourses = "Các khóa học";
$ManageCourses = "Quản lý các khóa học";
$EditCourses = "Chỉnh sửa các khóa học";
$CurrentCourses = "Các khóa học hiện tại";
$EditSessionCourses = "Chỉnh sửa các khóa học phiên";
$StudentCourses = "Các khóa học của người học";
$SessionCourses = "Các phiên khóa học";
$SubscribeClassToCourses = "Đăng ký lớp học để tham gia các khóa học";
$SubscribeGroupToCourses = "Đăng ký nhóm để tham gia các khóa học";
$ShowNumberOfCourses = "Hiển thị số khóa học";
$TabsMyCourses = "Tab khóa học";
$TotalNumberOfAvailableCourses = "Tổng số khóa học hiện có";
$CourseList = "Danh sách khóa học";
$CreateCourse = "Tạo một khóa học";
$AddCourse = "Tạo một khóa học";
$CourseCategories = "Các loại khóa học";
$AdminCategories = "Các loại khóa học";
$AddUsersToACourse = "Thêm người dùng vào khóa học";
$ImportUserListXMLCSV = "Nhập danh sách người dùng";
$ImportUsersToACourse = "Nhập danh sách người dùng";
$SearchCourse = "Tìm kiếm khóa học";
$SelectAnOption = "Vui lòng chọn một tùy chọn";
$AdvancedSearch = "Tìm kiếm nâng cao";
$SimpleCourseList = "Danh sách chuẩn";
$AdminCourseList = "Danh sách quản lý";
$LearnpathTitle = "Tiêu đề";
$Title = "Tiêu đề";
$ScormContentColumn = "Tiêu đề";
$WorkTitle = "Tiêu đề";
$ReceivedTitle = "Tiêu đề";
$Faculty = "Loại";
$Fac = "Loại";
$CourseFaculty = "Loại";
$Category = "Loại";
$ResourceType = "Loại";
$SubscriptionAllowed = "Registr. allowed";
$SelectAll = "Chọn tất cả";
$MarkAll = "Chọn tất cả";
$UnSelectAll = "Bỏ chọn tất cả";
$UnmarkAll = "Bỏ chọn tất cả";
$Actions = "Chi tiết";
$Detail = "Chi tiết";
$DeleteCourse = "Xóa (các) khóa học đã chọn";
$CourseCode = "Mã";
$OfficialCode = "Mã";
$SurveyCode = "Mã số";
$CourseTeachers = "Giáo viên";
$Trainers = "Giáo viên";
$CourseDepartment = "Trường/Khoa";
$Department = "Trường/Khoa";
$CourseTemplate = "Mẫu khóa học";
$FillWithExemplaryContent = "Điền vào nội dung demo";
$OpenToTheWorld = "Public - access allowed for the whole world";
$Unsubscribe = "Hủy đăng ký";
$CourseQuota = "Dung lượng đĩa";
$ThisFieldIsRequired = "Phần này không được để trống";
$AllowedToUnsubscribe = "Người dùng được phép hủy đăng ký khóa học này";
$NotAllowedToUnsubscribe = "Người dùng không được phép hủy đăng ký khóa học này";
$Denied = "Chức năng này chỉ có sẵn cho khóa đào tạo";
$OnlyLettersAndNumbers = "Chỉ các chữ cái (a-z) và số (0-9)";
$Option = "Lựa chọn";
$ExportAllCoursesList = "Xuất tất cả các khóa học";
$ExportSelectedCoursesFromCoursesList = "Xuất các khóa học đã chọn từ danh sách sau";
$OutputFileType = "Đầu ra của tệp";
$ImportCSVFileLocation = "Vị trí nhập tệp CSV";
$SubCat = "Danh mục phụ";
$FirstLetterUser = "Chữ cái đầu tiên (họ)";
$FirstLetterCourse = "Chữ cái đầu tiên (mã số)";
$AddToThatCourse = "Thêm vào khóa học";
$Course = "Khóa học";
$Training = "Khóa học";
$SearchButton = "Tìm kiếm";
$SearchPages = "Tìm kiếm";
$ToolSearch = "Tìm kiếm";
$DataTableSearch = "Tìm kiếm";
$Search = "Tìm kiếm";
$Sessions = "Các buổi học";
$ListSession = "Danh sách các buổi đào tạo";
$ListSessionCategory = "Danh sách các loại phiên";
$ImportSessionListXMLCSV = "Nhập danh sách phiên";


/** Portal */
$Platform = "Cổng thông tin";
$PlatformConfigSettings = "Thiết lập cấu hình";
$Languages = "Ngôn ngữ";
$Plugins = "Plugins";
$Regions = "Regions";
$SystemAnnouncements = "Cổng tin tức";
$GlobalAgenda = "Global agenda";
$ConfigureHomePage = "Chỉnh sửa trang chủ cổng thông tin";
$ConfigureInscription = "Thiết lập trang đăng ký";
$Statistics = "Thống kê";
$Stats = "Số liệu thống kê";
$Reports = "Báo cáo";
$TeacherTimeReport = "Báo cáo thời gian của giáo viên";
$ExtraFields = "Các trường bổ sung";
$PlatformLanguages = "Ngôn ngữ của cổng thông tin Chamilo";
$SignUp = "Đăng ký!";
$QuestionPlan = "Trợ giúp";
$Helptwo = "Trợ giúp";
$MenuGeneral = "Trợ giúp";
$HelpMaj = "Trợ giúp";
$InsertLink = "Thêm link liên kết";
$EditCategories = "Chỉnh sửa danh mục khóa học";
$LastName = "Họ đệm";
$FirstName = "Tên";
$Password = "Mật khẩu";
$SrcPhone = "Số điện thoại";
$Phone = "Điện thoại";
$ScormStatusColumn = "Trạng thái";
$ScormStatus = "Trạng thái";
$Status = "Trạng thái";
$ScormStatusColumn = "Trạng thái";
$Register = "Đăng ký";
$reg = "Đăng ký";
$RegisterUser = "Đăng ký";
$EditNotice = "Chỉnh sửa thông báo";
$LastAccess = "Lần truy cập mới nhất";
$LoginsByDate = "Đăng nhập theo ngày";
$UserStats = "Thống kê người dùng";
$UsersOnline = "Người dùng trực tuyến";
$Social = "Mạng xã hội";
$MessagesReceived = "Số lượng tin nhắn đã nhận";
$MessagesSent = "Số lượng tin nhắn đã gửi";
$CountFriends = "Số lượng liên hệ";
$ToolTracking = "Báo cáo";
$Reporting = "Báo cáo";
$Tracking = "Báo cáo";
$MySpace = "Báo cáo";
$Report = "Báo cáo";


/** System */
$System = "Hệ thống";
$ArchiveDirCleanup = "Dọn dẹp bộ nhớ cache và các tệp tạm thời";
$SpecialExports = "Special exports";
$SystemStatus = "Tình trạng hệ thống";
$TicketSystem = "Tickets";
$Tickets = "Tickets";
$SpecialCreateFullBackup = "Special create full backup";
$SpecialLetMeSelectItems = "Special let me select items";
$Reset = "Cài lại";


/** Người dùng */
$CourseDetails = "Mô tả khóa học";
$ToolCourseDescription = "Mô tả khóa học";
$ToolCalendarEvent = "Lịch học";
$GroupCalendar = "Agenda";
$GroupDocument = "Tài liệu tham khảo";
$Documents = "Tài liệu tham khảo";
$DocumentsAccess = "Tài liệu tham khảo";
$MdCallingTool = "Tài liệu tham khảo";
$ToolDocument = "Tài liệu tham khảo";
$ToolLearnpath = "Danh sách bài học";
$ToolQuiz = "Thi trắc nghiệm";
$Tests = "Thi trắc nghiệm";
$Exercises = "Bài kiểm tra";
$Quiz = "Bài kiểm tra";
$Users = "Quản lý người dùng";
$NbUsers = "Users";
$BredCrumpUsers = "Users";
$ToolUser = "Danh sách học sinh";
$StudentPublications = "Bài tập";
$GroupWork = "Assignments";
$Student_publication = "Assignments";
$AvgAssignments = "Assignments";
$Assignment = "Bài tập về nhà";
$ToolStudentPublication = "Bài tập về nhà";
$CurrentDirectory = "Thư mục hiện thời";
$NoDocsInFolder = "Không có tài liệu";
$LearningPath = "Bài học";
$LearningPaths = "Bài học";
$Professor = "Giáo viên";
$Teacher = "Giáo viên";
$Photo = "Ảnh";
$Login = "Đăng nhập";
$LoginEnter = "Đăng nhập";
$LoginAsThisUser = "Login";
$UserOnHost = "Login";
$LoginName = "Tên đăng nhập";
$GroupSingle = "Nhóm";
$Group = "Nhóm";
$RoleType = "Loại";
$Type = "Loại";
$HandOutDateLimit = "Hạn nạp";
$FeedbackType = "Đánh giá, nhận xét của giáo viên";
$Feedback = "Đánh giá, nhận xét của giáo viên";
$LastUpload = "Thời gian nạp";
$AgendaList = "Danh sách các sự kiện";
$EditProfile = "Sửa thông tin cá nhân";
$Email = "Email";
$email = "Email";
$AddImage = "Ảnh đại diện";
$OnlyImagesAllowed = "Chỉ được dùng ảnh dạng PNG, JPG hoặc GIF";
$UserPassword = "Mật khẩu";
$TypeCurrentPassword = "Để thay đổi mật khẩu, Bạn nhập mật khẩu hiện thời vào ô phía trên và nhập 2 lần mật khẩu mới vào 2 ô phía dưới";
$NewPass = "Mật khẩu mới";
$SaveSettings = "Lưu thiết lập";
$AddCustomCourseIntro = "Bạn có thể thêm phần giới thiệu về khóa học này tại đây bằng cách nhấp vào biểu tượng ấn bản";
$SwitchToStudentView = "Chuyển sang chế độ xem sinh viên";
$SwitchToTeacherView = "Chuyển sang chế độ xem dành cho giáo viên";
$Vote = "Đánh giá";
$CourseHomepage = "Đăng ký";
$QuantityQuestions = "Câu hỏi";
$StartTest = "Bắt đầu kiểm tra";
$NextQuestion = "Câu hỏi tiếp theo";
$PreviousQuestion = "Câu hỏi trước";
$YouTriedToResolveThisExerciseEarlier = "Bạn đã cố gắng giải bài tập này trước đó";
$EndTest = "Kết thúc bài kiểm tra";
$AnotherAttempt = "Another attempt";
$ContinueTest = "Tiến hành kiểm tra";
$Visits = "Lượt truy cập";
$YourVote = "Đánh giá của bạn";
$ShareWithYourFriends = "Chia sẻ với bạn bè của bạn";
$CourseInformation = "Thông tin khóa học";
$Coaches = "Giáo viên";
$RegistrationRoleWhatDoYouWantToDo = "Bạn muốn làm gì?";
$MailHasBeenSent = "Một email đã được gửi để nhắc bạn về thông tin đăng nhập và mật khẩu của bạn";
$PasswordWeak = "Yếu";
$PasswordStrong = "Mạnh";
$PasswordMedium = "Trung bình";
$PasswordNormal = "Bình thường";
$PasswordVeryWeak = "Rất yếu";
$PasswordVeryStrong = "Rất mạnh";
$EnterYourNewPassword = "Nhập mật khẩu mới của bạn ở đây.";
$RepeatYourNewPassword = "Nhập mật khẩu mới của bạn một lần nữa để giảm khả năng xảy ra lỗi khi nhập.";
$HelloXAsYouCanSeeYourCourseListIsEmpty = "<p>Xin chào <strong>%s</strong> </p>
<p>Như bạn có thể thấy, danh sách các khóa học của bạn vẫn còn trống. Đó là bởi vì bạn chưa đăng ký bất kỳ khóa học nào! </p>";
$GoAheadAndBrowseOurCourseCatalogXOnceRegisteredYouWillSeeTheCourseHereX = "<p>Hãy tiếp tục và duyệt qua danh mục khóa học của chúng tôi %s để đăng ký bất kỳ khóa học nào bạn thích. Sau khi đăng ký, bạn sẽ thấy khóa học hiện ra ngay %s, thay cho tin nhắn này.</p>";
$SortKeys = "Sắp xếp theo";
$NumberOfMatchingCourses = "Số lượng khóa học phù hợp";
$YouNotYetAchievedCertificates = "You have not achieved any certificate just yet. Continue on your learning path to get one!";
$Score = "Điểm";
$Details = "Chi tiết";
$Show = "Hiện thị";
$NotValidated = "Đã làm bài";
$Correct = "Đúng";
$XAnswersSavedByUsersFromXTotal = "%d / %d câu trả lời đã được lưu.";
$BackToAttemptList = "Quay lại bài kiểm tra";
$Duration = "Số phút";
$NotAttempted = "Chưa làm";
$AnotherAttempt = "Nhấn vào đây để thi lượt khác";
$ReturnToCourseHomepage = "Trở về khóa học";
$Votes = "Đánh giá";
$TheDocumentAutoLaunchSettingIsOnStudentsWillBeRedirectToTheDocumentTool = "Cấu hình tính năng tự động khởi chạy tài liệu được bật. Người học sẽ được tự động chuyển hướng đến công cụ tài liệu.";
$Visit = "Lượt truy cập";
$LoginOrEmailAddress = "Tên đăng nhập hoặc Email";
$EnterEmailUserAndWellSendYouPassword = "Nhập tên người dùng hoặc địa chỉ e-mail mà bạn đã đăng ký và chúng tôi sẽ gửi mật khẩu của bạn.";
$Send = "Gửi tin nhắn";
?>